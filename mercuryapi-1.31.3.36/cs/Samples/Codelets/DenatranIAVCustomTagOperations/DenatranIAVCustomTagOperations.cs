...
namespace DenatranIAVCustomTagOperations
{
    ...
    class DenatranIAVCustomTagOperations
    {
        ...
        //IAVDenatran secure tag operation - G0-PA Auth
        Gen2.Denatran.IAV.G0PAOBUAuth tagOpG0PAOBUAuth = null;
        //IAVDenatran secure tag operation - G0-PA Full Pass
        Gen2.Denatran.IAV.G0PAOBUAuth tagOpG0PAOBUAuthFullPass = null;
        //IAVDenatran secure tag operation - G0-PA Full Pass1
        Gen2.Denatran.IAV.G0PAOBUAuth tagOpG0PAOBUAuthFullPass1 = null;

        Reader reader;
        // antenna list to opearate
        ...
        private void ExecuteTagOpFilter(TagFilter filter)
        {
            try
            {
                ...
                //G0-PA commands
                byte[] G0PAOBUAuthdata = (byte[])reader.ExecuteTagOp(tagOpG0PAOBUAuth, filter);
                Console.WriteLine("G0PAOBUAuthdata mode response data : {0}", ByteFormat.ToHex(G0PAOBUAuthdata));
                Console.WriteLine("Data length : " + G0PAOBUAuthdata.Length);
                Console.WriteLine();

                //G0-PA commands
                byte[] G0PAOBUAuthFullPass = (byte[])reader.ExecuteTagOp(tagOpG0PAOBUAuthFullPass, filter);
                Console.WriteLine("G0PAOBUAuthFullPass mode response data : {0}", ByteFormat.ToHex(G0PAOBUAuthFullPass));
                Console.WriteLine("Data length : " + G0PAOBUAuthFullPass.Length);
                Console.WriteLine();

                //G0-PA commands
                byte[] G0PAOBUAuthFullPass1 = (byte[])reader.ExecuteTagOp(tagOpG0PAOBUAuthFullPass1, filter);
                Console.WriteLine("G0PAOBUAuthFullPass1 mode response data : {0}", ByteFormat.ToHex(G0PAOBUAuthFullPass1));
                Console.WriteLine("Data length : " + G0PAOBUAuthFullPass1.Length);
                Console.WriteLine();
                ...
                ReadSec = null;
                WriteSec = null;
                G0PAOBUAuthdata = null;
                G0PAOBUAuthFullPass = null;
                G0PAOBUAuthFullPass1 = null;
            }
            catch (ReaderException re)
            {
                throw re;
